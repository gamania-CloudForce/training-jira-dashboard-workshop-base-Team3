...
// 模擬 Hook 返回值的生成函數
function createMockSprintData(
  overrides: Partial<MockSprintBurndownData> = {}
): MockSprintBurndownResponse {
...
describe("Sprint 燃盡圖資料邏輯測試", () => {
  ...
  describe("TC-001-01: 正常顯示燃盡圖與完成率", () => {
    ...
    test("應該計算正確的剩餘故事點", () => {
      ...
      // 驗證數學關係：總計 = 已完成 + 剩餘
      expect(
        sprint_data.completed_story_points + sprint_data.remaining_story_points
