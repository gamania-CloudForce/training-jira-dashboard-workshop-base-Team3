...
function calculateProgressHealth(completionRate: number, timeProgressRate: number): 'normal' | 'warning' | 'danger' {
...
// 計算時間進度率
function calculateTimeProgress(daysElapsed: number, totalWorkingDays: number): number {
...
describe('Sprint 數據整合測試', () => {
  ...
  test('TC-001-01 數據應該返回正確的健康狀態', () => {
    ...
    const completionRate = (sprintData.completedStoryPoints / sprintData.totalStoryPoints) * 100; // 65%
    const timeProgress = calculateTimeProgress(sprintData.daysElapsed, sprintData.totalWorkingDays); // 70%
  ...
  test('TC-001-03 數據應該返回警示狀態', () => {
    ...
    const completionRate = (sprintData.completedStoryPoints / sprintData.totalStoryPoints) * 100; // 50%
    const timeProgress = calculateTimeProgress(sprintData.daysElapsed, sprintData.totalWorkingDays); // 70%
    const healthStatus = calculateProgressHealth(completionRate, timeProgress);
  ...
  test('TC-001-04 數據應該返回危險狀態', () => {
    ...
    const completionRate = (sprintData.completedStoryPoints / sprintData.totalStoryPoints) * 100; // 30%
    const timeProgress = calculateTimeProgress(sprintData.daysElapsed, sprintData.totalWorkingDays); // 80%
    const healthStatus = calculateProgressHealth(completionRate, timeProgress);
