...
test.describe("Sprint 燃盡圖 E2E 測試", () => {
   ...
   * 對應後端測試案例: TC-001-01
   */
  test("TC-001-E2E-01: 應該正確顯示 Sprint 燃盡圖與完成率", async ({
    page,
  }) => {
    // 1. 選擇特定 Sprint
    ...
    await page.waitForSelector(
      '[data-testid="completion-rate"], text="Sprint 完成率"',
      { timeout: 10000 }
    );

    // 3. 驗證完成率卡片顯示
    const completionRateCard = page
      .locator('text="Sprint 完成率"')
      .locator("..");
  ...
  test("TC-001-E2E-02: 應該正確顯示進度健康狀態", async ({ page }) => {
    ...
    const healthStatusExists =
      (await page.locator("text=/正常進度|稍微落後|嚴重落後/").count()) > 0;
    expect(healthStatusExists).toBeTruthy();

    // 檢查完成率百分比是否顯示
    const percentageRegex = /\d+%/;
    const percentageElement = page
      .locator("text=" + percentageRegex.source)
      .first();
  ...
  test("TC-001-E2E-03: 應該正確處理 Sprint 切換", async ({ page }) => {
    ...
    if (allExists) {
      ...
      await expect(
        page.locator("text=/請選擇一個特定的 Sprint/i")
      ).toBeVisible();
    }
  ...
  test("TC-001-E2E-04: 應該正確顯示載入狀態", async ({ page }) => {
    ...
    const sprintOptions = page.locator('[role="option"]');
    const firstNonAllOption = sprintOptions
      .locator("text=/DEMO1|Sprint/")
      .first();
  ...
  test("TC-001-E2E-07: 應該正確整合後端 API", async ({ page }) => {
    // 1. 直接測試 API 端點
    const sprintListResponse = await page.request.get(
      `${BACKEND_URL}/api/sprint/list`
    );
    ...
    if (sprintList.sprints.length > 0) {
      ...
      const burndownResponse = await page.request.get(
        `${BACKEND_URL}/api/sprint/burndown/${encodedSprintName}`
      );
...
test.describe("Sprint 燃盡圖錯誤處理測試", () => {
  test("TC-001-ERROR-01: 網路錯誤處理測試", async ({ page }) => {
    ...
    if ((await sprintOption.count()) > 0) {
      ...
      // 應該顯示錯誤訊息
      await expect(page.locator("text=/載入失敗|錯誤|Error/i")).toBeVisible({
        timeout: 10000,
      });
    }
  });
