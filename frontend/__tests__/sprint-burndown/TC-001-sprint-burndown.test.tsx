...
describe("Sprint Burndown Visualization - Test Cases", () => {
  ...
  describe("TC-001-01: 正常顯示燃盡圖與完成率", () => {
    ...
    test("完整容器整合測試", () => {
      renderWithTheme(<SprintBurndownContainer selectedSprint="Sprint 2" />);
  ...
  describe("TC-001-05: 進度狀態邊界值測試", () => {
    test("應該正確處理邊界值狀態", () => {
      ...
      renderWithTheme(<CompletionRateCard sprintData={boundaryWarningData} />);
    ...
    test("應該正確處理危險邊界值", () => {
      ...
      renderWithTheme(<CompletionRateCard sprintData={boundaryDangerData} />);
  ...
  describe("載入和錯誤狀態", () => {
    ...
    test("應該正確顯示錯誤狀態", () => {
      ...
      expect(
        screen.getByText(/API Error: Failed to fetch data/)
      ).toBeInTheDocument();
    });

    test("選擇 All 時應該顯示提示訊息", () => {
      renderWithTheme(<SprintBurndownContainer selectedSprint="All" />);

      expect(
        screen.getByText("請選擇一個特定的 Sprint 來查看燃盡圖和完成率分析")
      ).toBeInTheDocument();
    });
  });
});
