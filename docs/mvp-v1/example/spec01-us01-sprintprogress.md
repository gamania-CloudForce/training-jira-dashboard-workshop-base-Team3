...
### US-001: Sprint 燃盡圖視覺化（含 Sprint 完成率）

**US001**: 作為 Scrum Master，我希望能在儀表板上看到即時更新的 Sprint 燃盡圖，並清楚顯示目前 Sprint 的完成率（如：65% 完成，13/20 故事點數），同時根據進度健康狀態顯示不同顏色的進度條和燃盡線，以便在每日站會中快速識別進度是否落後於預期，並引導團隊討論真正的障礙。
...
#### AC01: 正常顯示 Sprint 完成率與燃盡圖
```gherkin
場景：團隊成員於儀表板查看 Sprint 進度
Given Google Sheets 已有正確的 Sprint 相關資料（故事點數、狀態等）
And 後端 API 能正確回傳 Sprint 進度與完成率統計
When Scrum Master 或團隊成員開啟儀表板
Then 頁面應顯示最新的 Sprint 燃盡圖與完成率（如：65% 完成，13/20 故事點數）
And 完成率與圖表數據需與 Google Sheets 資料一致
```

#### AC02: 進度健康度顏色指示
```gherkin
場景：進度健康度視覺化
Given 已正確顯示 Sprint 燃盡圖與完成率
When Sprint 進度健康度達到不同門檻（如落後、警示、正常）
Then 進度條與燃盡線應分別以紅色、黃色、綠色顯示對應狀態
And 使用者能一目了然辨識進度風險
```
...
```gherkin
場景：Sprint 資料異常或缺漏
Given Google Sheets Sprint 資料有缺漏或格式錯誤
Or 後端 API 回傳異常
When 使用者開啟儀表板
Then 頁面應顯示友善的錯誤訊息或預設提示
And 不應顯示錯誤或誤導性的進度數據
```
